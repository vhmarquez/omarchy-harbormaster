...
License selection is pending the repository owner's explicit response. MIT is recommended for simplicity; Apache-2.0 is an alternative with explicit patent language. No project license grant is published by this ADR. Third-party design font retains its upstream OFL and is not relicensed. Dependency/license compatibility must be audited against actual pinned dependencies in M1, especially Qt/Quickshell and distribution obligations; selecting a permissive project license does not remove those obligations.

## Consequences and gates

Proceeding into M1 requires the M0 exit record: runtime feasibility, per-harness limitations, approved design baseline, domain/performance contracts and license choice. Routine reversible implementation details may be settled by the implementation owner. M0 does not install a runtime or claim production readiness. Source/version inspection and fixture tests are labeled separately from actual harness execution.
