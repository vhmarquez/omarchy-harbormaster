...
pub(super) fn initialize(conn: &mut Connection) -> Result<(), StorageError> {
    ...
    tx.execute_batch("INSERT INTO maintenance VALUES(1,0,0,0,0); PRAGMA application_id=1212304689; PRAGMA user_version=3;")?;
    tx.commit()?;
    Ok(())
}
...
pub(super) fn inspect(conn: &Connection) -> Result<i64, StorageError> {
    ...
    let mut query = conn.prepare(
        "SELECT name,sql FROM sqlite_schema WHERE name NOT LIKE 'sqlite_%' ORDER BY name LIMIT 10",
    )?;
    let actual = query
        .query_map([], |row| {
