...
use harbormaster::protocol::{EventHandshake, HarnessKind, ProducerGeneration, Seq, parse_event_handshake};
use std::fs;
...
impl Fixture {
    fn new(channel: Channel) -> Self {
        let path = std::env::temp_dir().join(format!("hb-ingestion-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
        fs::DirBuilder::new().mode(0o700).create(&path).unwrap();
        let sockets = PrivateSockets::bind(&path).unwrap();
        let filename = if channel == Channel::Event { "events.sock" } else { "control.sock" };
        let client = UnixStream::connect(path.join("harbormaster").join(filename)).unwrap();
        let budget = ConnectionBudget::new(64).unwrap();
        let peer = sockets.accept(channel, &budget, Instant::now() + Duration::from_secs(5)).unwrap().unwrap();
        Self { _client: client, peer, _sockets: sockets, path }
    }
}
...
fn registered(peer: &Connection) -> Registry {
    ...
    registry.register(ProducerRegistration {
        producer_id: h.producer_id, generation: h.generation, run_id: h.run_id,
        harness: HarnessKind::Hermes, uid: peer.peer().uid(),
        allowed_signals: h.requested_signals, next_sequence: Seq::new(1),
    }).unwrap();
    registry
}
...
fn admission_is_bounded_and_duplicates_never_count_as_durable_acknowledgment() {
    ...
    assert_eq!(registry.admit(&session, &first), Ok(Admission::AlreadyAdmitted));
...
fn wrong_channel_generation_producer_run_and_signal_cannot_queue() {
    ...
    assert!(matches!(registry.connect(&f.peer, &handshake()), Err(AdmissionError::PermissionDenied)));
    let f = Fixture::new(Channel::Event);
    let mut registry = registered(&f.peer);
    let session = registry.connect(&f.peer, &handshake()).unwrap();
    for (before, after) in [(GENERATION, "55555555-5555-4555-8555-555555555555"),
                           (PRODUCER, "55555555-5555-4555-8555-555555555555"),
                           (RUN, "55555555-5555-4555-8555-555555555555"),
                           ("turn.started", "turn.completed")] {
        let input = String::from_utf8(event(1, 1, GENERATION)).unwrap().replace(before, after);
...
fn unknown_generation_cannot_reconnect_or_reset_sequence() {
    ...
    assert!(matches!(registry.connect(&f.peer, &h), Err(AdmissionError::StaleGeneration)));
    let empty = Registry::new();
    assert!(matches!(empty.connect(&f.peer, &handshake()), Err(AdmissionError::UnknownProducer)));
}

#[test]
fn sequence_gap_and_conflict_require_trusted_reconciliation() {
    for bad in [event(3, 3, GENERATION), event(1, 2, GENERATION), event(2, 1, GENERATION)] {
        let f = Fixture::new(Channel::Event);
        let mut registry = registered(&f.peer);
        let session = registry.connect(&f.peer, &handshake()).unwrap();
        registry.admit(&session, &event(1, 1, GENERATION)).unwrap();
        assert!(registry.admit(&session, &bad).is_err());
        assert_eq!(registry.admit(&session, &event(2, 2, GENERATION)), Err(AdmissionError::ReconciliationRequired));
        assert_eq!(registry.queued(), 1);
    }
}
...
fn per_producer_backpressure_does_not_advance_sequence() {
    ...
    let mut registry = registered(&f.peer);
    let session = registry.connect(&f.peer, &handshake()).unwrap();
    for i in 1..=128 { registry.admit(&session, &event(i, i, GENERATION)).unwrap(); }
    assert_eq!(registry.admit(&session, &event(129, 129, GENERATION)), Err(AdmissionError::QueueFull));
    registry.pop().unwrap();
    assert_eq!(registry.admit(&session, &event(129, 129, GENERATION)), Ok(Admission::Queued));
...
fn fresh_generation_discards_backlog_and_invalidates_old_session() {
    ...
    let session = registry.connect(&f.peer, &handshake()).unwrap();
    registry.admit(&session, &event(1, 1, GENERATION)).unwrap();
    let generation: ProducerGeneration = "55555555-5555-4555-8555-555555555555".parse().unwrap();
    registry.reconcile(handshake().producer_id, generation.clone(), Seq::new(1)).unwrap();
    assert_eq!(registry.queued(), 0);
    assert_eq!(registry.discarded(), 1);
    assert_eq!(registry.admit(&session, &event(2, 2, GENERATION)), Err(AdmissionError::StaleGeneration));
    let mut h = handshake(); h.generation = generation.clone();
    let session = registry.connect(&f.peer, &h).unwrap();
    assert_eq!(registry.admit(&session, &event(1, 3, &generation.to_string())), Ok(Admission::Queued));
}
...
fn malformed_content_is_not_retained_or_admitted() {
    ...
    let mut registry = registered(&f.peer);
    let session = registry.connect(&f.peer, &handshake()).unwrap();
    let malformed = String::from_utf8(event(1, 1, GENERATION)).unwrap().replace("\"turn_id\":", "\"prompt\":\"SYNTHETIC-PRIVATE-CONTENT\",\"turn_id\":");
    let error = registry.admit(&session, malformed.as_bytes()).unwrap_err();
    assert!(!format!("{error:?} {error}").contains("SYNTHETIC"));
    assert_eq!(registry.queued(), 0);
    assert_eq!(registry.admit(&session, &event(1, 1, GENERATION)), Ok(Admission::Queued));
}
