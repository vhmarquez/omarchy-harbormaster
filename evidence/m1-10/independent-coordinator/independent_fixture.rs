...
fn until_apply(job: &mut CommitJob<'_>) {
    ...
    while !matches!(job.purpose, super::super::commit::Purpose::Apply) {
        ...
        assert!(job.take_committed().is_none(), "no receipt before Apply submission");
...
fn independent_receipt_exists_only_after_observed_atomic_commit() {
    ...
    let mut job = CommitJob::new(&fixture.worker, &mut fixture.registry, entry, policy(), 7, true);
    assert!(job.take_committed().is_none());
    until_apply(&mut job);
    let before = fixture.worker.try_submit(Request::Status).unwrap()
        .wait_timeout(Duration::from_secs(3)).unwrap();
    let Response::Status(before) = before else { panic!("status"); };
    assert_eq!((before.facts, before.tombstones, before.pending_deliveries), (0, 0, 0));
    assert!(job.take_committed().is_none());
    assert_eq!(finish(&mut job), CommitProgress::Committed { revision: Revision::new(2) });
    let (receipt, entry) = job.take_committed().unwrap();
    assert_eq!(receipt.event(), &event);
    assert!(job.take_committed().is_none());
    drop(job);
    let status = fixture.status();
    assert_eq!((status.facts, status.tombstones, status.attention, status.pending_deliveries), (1, 1, 1, 1));
...
fn independent_same_identity_changed_payload_gets_no_receipt() {
    ...
    let entry = artifacts::read_synthetic(&other_store, 2000).into_iter()
        .find(|entry| entry.event().event.kind() == EventKind::TurnStarted).unwrap();
...
fn independent_exact_receipt_does_not_bypass_source_harness_scope() {
    ...
    let (progress, receipt) = fixture.commit(entry);
    assert_eq!(progress, CommitProgress::Rejected(CommitFailure::Unreconciled));
    assert!(receipt.is_none());
    assert_eq!(other_store.snapshot().unwrap().spool_files, 1);
}
